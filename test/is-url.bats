...
@test "success for simple HTTPS URL" {
    run is-url "http://www.google.com/" 2>&1
    assert_success
    assert_output ""
}

@test "success for HTTPS URL with one argument" {
    run is-url "https://www.google.com/search?q=biapy" 2>&1
    assert_success
    assert_output ""
}

@test "success for HTTPS URL with two arguments" {
    run is-url "https://www.google.com/search?q=biapy&hl=en" 2>&1
    assert_success
    assert_output ""
}

@test "success for well formatted unexistant URL" {
    run is-url "https://www.my-random-broken-and-inexistant-site.bogus/" 2>&1
    assert_success
    assert_output ""
}
...
@test "success for simple FTP url" {
    run is-url "ftp://www.google.com/" 2>&1
    assert_success
    assert_output ""
}

@test "success for simple HTTPS url with login and password" {
    run is-url "https://username:password@www.google.com/" 2>&1
    assert_success
    assert_output ""
}

@test "success for simple FILE url" {
    run is-url "file:///home/" 2>&1
    assert_success
    assert_output ""
}

@test "fail on simple SSH url" {
    run is-url "ssh://www.google.com/" 2>&1
    assert_failure
    assert_output ""
}

@test "fail on simple git url" {
    run is-url "git://github.com/biapy/biapy-bashlings.git" 2>&1
    assert_failure
    assert_output ""
}
